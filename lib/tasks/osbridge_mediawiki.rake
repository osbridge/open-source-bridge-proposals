...
For example:
    ...
    task :populate => :environment do
      require 'media_wiki_gateway_page_drone'
      require 'media_wiki'

      credentials = get_wiki_credentials
      #wiki = RWikiBot.new(credentials.user, credentials.password, credentials.url, '', true)
      wiki = MediaWiki::Gateway.new(credentials.url)
      wiki.login(credentials.user, credentials.password)
      event = ENV['EVENT'] ? OpenConferenceWare::Event.find_by_slug(ENV['EVENT']) : OpenConferenceWare::Event.current

      # Event page
      drone = MediaWikiGatewayPageDrone.new(wiki, event_wiki_title(event))
      drone.replace_span "description", "For details, see [#{OpenConferenceWare.public_url} #{OpenConferenceWare.organization}]."
      drone.save(true)

      # Tracks page
      drone = MediaWikiGatewayPageDrone.new(wiki, tracks_wiki_title(event))
      ...
      event.tracks.includes(:event).each do |track|
        drone = MediaWikiGatewayPageDrone.new(wiki, track_wiki_title(track))
      ...
      event.proposals.confirmed.includes(:event, :track, :session_type).each do |proposal|
        drone = MediaWikiGatewayPageDrone.new(wiki, session_wiki_title(proposal))
